...
Localizer::Localizer(unsigned int nb_cam ):n("~")
{
    SetNbCamera(nb_cam);
...
void Localizer::AddReferenceStaticMarkers(const YAML::Node& node)
{
    ...
    for (YAML::const_iterator it=config.begin();it!=config.end();++it) 
    {
        std::cout<<"adding one static marker"<<std::endl;
...
{
    std::cout<<"PublishObject"<<std::endl;
    ...
    {
        std::cout<<"One object defined"<<std::endl;
...
marker Localizer::ReadMarkerInfo( const YAML::Node& node)
{
    ...
    std::cout<<"Reading ID = "<< out.id<<std::endl;
                                            ...
                                            unsigned int camera_id)
{
    ...
    // on localise la caméra par rapport au premier marqueur vu.
    int first_id_seen = -1;
    for (int i=0;i<reference_markers.size();i++)
    {
        if (FoundMarker(msg,reference_markers[i].id,tmp))   if (reference_markers[i].already_seen)    // on vérifie qu'on a deja vu le marqueur
        {
            first_id_seen = i;           
            cameras_poses[camera_id] = reference_markers[i].pose * tmp.inverse();          
    ...
    // maintenant on localise tous les marqueurs fixes dans le repere 1 (si ils ne sont pas connus)
    for (int i=first_id_seen+1;i<reference_markers.size();i++)
    {
        if (FoundMarker(msg,reference_markers[i].id,tmp))   if (reference_markers[i].known_reference == false)
        {
            reference_markers[i].already_seen = true;                    
            reference_markers[i].pose = cameras_poses[camera_id]  * tmp;
        }
    }      
    ...
    for (int i=0;i<objects.size();i++)
    {
        ...
        for (int j=0;j<nb;j++)
        {
            if( FoundMarker(msg,ids[j],tmp))
            {
                Transformation local_pose = objects[i].GetLocalPose(j);                
                Transformation object_pose = cameras_poses[camera_id] * tmp * local_pose.inverse();
